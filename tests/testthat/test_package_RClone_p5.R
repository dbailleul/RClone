...
test_that("the autocorrelation function works", {
	...
	prev <- autocorrelation_core(posidonia, Loiselle = TRUE, coords = coord_posidonia)
	
	expect_that(prev, is_a("list"))
	expect_that(prev[[1]], is_a("data.frame"))
	expect_equal(length(prev), 5)
	expect_equal(ncol(prev[[1]]), 6)
	
	res1 <- autocorrelation(posidonia, Loiselle = TRUE, coords = coord_posidonia, vecdist = c(0,10,15,20,30,50,70,76.0411073))
	...
	res4 <- autocorrelation(posidonia, Ritland = TRUE, coords = coord_posidonia, nbrepeat = 10)
...
test_that("the aggregation function works", {
	...
	res <- agg_index(posidonia, coord_posidonia)
	
	expect_that(res, is_a("list"))
	expect_that(res[[1]], is_a("matrix"))
	expect_equal(nrow(res[[1]]), 1)
	expect_equal(ncol(res[[1]]), 3)
	expect_equal(round(res[[1]][1,1], digits = 5)[[1]], round(0.2272127, digits = 5), tolerance = 0.00001)
...
test_that("the edge effect function works", {
	...
	res <- edge_effect(posidonia, coords = coord_posidonia, center = c(40,10))
	
	expect_that(res, is_a("list"))
	expect_that(res[[1]], is_a("data.frame"))
	expect_equal(nrow(res[[1]]), 1)
	expect_equal(ncol(res[[1]]), 3)
	expect_equal(round(res[[1]][1,1], digits = 5)[[1]], round(0.07786722, digits = 5), tolerance = 0.00001)
