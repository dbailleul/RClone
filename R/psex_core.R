...
psex_core <- function (data1, data2, haploid = FALSE, MLGsim = FALSE, nbrepeat = NULL, 
    bar = FALSE) 
{
    if (nrow(data1) == nrow(unique(data1))) {
		...
		if (haploid == TRUE){
			...
			if (haploid == TRUE){
                  	...
                  	for (j in 1:N) {
                    		for (i in index_l) {
                      			tab2[j, i] <- sort(sample(split(data2, 
                       			 liste_all)[[i]][, ncol_all], 
                        		1, prob = split(data2, liste_all)[[i]][, ncol_freq], 
						replace = TRUE))
                    		}
                  	}
                ...
                colnames(psexFR_p) <- c("genet", "psex", 
                  "pvalue")
                if (bar) {
                  close(pb)
                }
                ...
                if (length(psex_recup) >= 1 & length(psex_recup) < 
                  100) {
