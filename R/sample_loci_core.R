...
sample_loci_core <- function (data1, data2, haploid = FALSE, nbrepeat = 1000, He = FALSE, 
    graph = FALSE, bar = FALSE) 
{
    if (He & haploid) {
        stop("Haploids and He computations are not compatible.")
    }
    if (haploid) {
    ...
    if (He) {
        He_box_l <- as.data.frame(matrix(NA, ncol = nb_loci, 
            nrow = nbrepeat))
        He_box_l2 <- as.data.frame(matrix(NA, ncol = nb_loci, 
            nrow = nbrepeat))
    ...
    for (j in 1:nbrepeat) {
        for (i in index_l) {
            indic <- sample(index_l, which(index_l == i), replace = FALSE)
            mat_test <- data1[, sort(c(indic, indic + 1))]
            if (He) {
                He_ind <- NULL
                for (l in indic) {
                  He_ind <- c(He_ind, 2 * nrow(mat_test)/(2 * 
                    nrow(mat_test) - 1) * (1 - sum(as.numeric(data2[rownames(data2[data2 == 
                    paste("locus", c((l + 1)/2), sep = "_"), 
                    ]), ncol_freq])^2)))
                }
                He_box_l[j, c((i + 1)/2)] <- mean(He_ind)
                He_box_l2[j, c((i + 1)/2)] <- var(He_ind)
                colnames(He_box_l) <- colnames(He_box_l2) <- c("1_locus", 
                  paste(2:nb_loci, "loci", sep = "_"))
            }
            mat_test <- unique(mat_test)
            mat_box_l[j, c((i + 1)/2)] <- nrow(mat_test)
            nb_loc <- NULL
            for (k in index_l[1:c(ncol(mat_test)/2)]) {
                nb_loc <- c(nb_loc, length(unique(unlist(mat_test[, 
                  c(k, k + 1)]))))
            }
    ...
    colnames(mat_box_l) <- colnames(mat_box_l2) <- colnames(mat_box_l3) <- c("1_locus", 
        paste(2:nb_loci, "loci", sep = "_"))
    ...
    colnames(mat_res_MLG_l) <- c("nb_loci", "min", 
        "max", "mean_MLG", "SE")
    ...
    if (He) {
        ...
        colnames(mat_res_all_l) <- c("nb_loci", "min", 
            "max", "mean_all", "SE", "He", 
            "SE")
    }
    else {
    ...
    if (He) {
        res <- list(res_MLG = mat_res_MLG_l, res_alleles = mat_res_all_l, 
            raw_He = He_box_l, raw_MLG = mat_box_l, raw_all = mat_box_l2)
    }
    else {
        res <- list(res_MLG = mat_res_MLG_l, res_alleles = mat_res_all_l, 
            raw_MLG = mat_box_l, raw_all = mat_box_l2)
    }
    res
}
